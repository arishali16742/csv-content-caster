...
interface MiddleEastDestination {
  id: string;
  name: string;
  emoji: string;
  image: string | null;
...
const MiddleEast = () => {
  ...
  const [destinations, setDestinations] = useState<MiddleEastDestination[]>([]);
  ...
  const sectionTitle = getSectionTitle('middle-east', 'EXPLORE MIDDLE EAST');
  const sectionSubtitle = getSectionSubtitle('middle-east', 'Discover amazing destinations in the Middle East collection');

  return (
    <section className="py-12 md:pt-[1.5rem] md:pb-4 bg-white">
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 md:mb-8 space-y-4 md:space-y-0">
          ...
          <div className="flex gap-2 mx-auto md:mx-0">
            ...
            >
              <ChevronLeft className="h-5 w-[2rem]" />
            </button>
            <button 
              onClick={handleNext} 
              disabled={!canGoNext} 
            ...
            >
              <ChevronRight className="h-5 w-[2rem]" />
            </button>
          </div>
        </div>
