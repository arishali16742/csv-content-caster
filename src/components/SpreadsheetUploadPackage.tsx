...
const SpreadsheetUploadPackage = ({ onDataParsed, expectedColumns, title, templateData }: SpreadsheetUploadProps) => {
  ...
  const parseCSV = (text: string) => {
    ...
    return rows.map((row, index) => {
      ...
      const titleValue = obj.title || obj.name;
      return obj && titleValue && titleValue.trim() !== '';
    });
  };
  ...
  const validateData = (data: any[]) => {
    ...
    data.forEach((row, index) => {
      const titleValue = row.title || row.name;
      if (!titleValue || titleValue.trim() === '') {
        errors.push(`Row ${index + 1}: Title/Name is required`);
      }
      ...
      if (isNaN(row.original_price) || row.original_price < 0) {
        errors.push(`Row ${index + 1}: Invalid original_price value`);
      }
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-sm text-yellow-600 mb-2">
            ⚠️ Important: Ensure all price fields contain only numbers (no decimal points for price/original_price), 
...
export default SpreadsheetUploadPackage;